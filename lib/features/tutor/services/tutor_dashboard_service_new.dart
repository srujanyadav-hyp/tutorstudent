      ...
      for (int i = 6; i >= 0; i--) {
        ...
        for (final session in sessions) {
          ...
          if (sessionDate.year == date.year &&
            ...
            dailySessionCount++;
            dailyEarnings += (session['amount'] as num).toDouble();
          }
        }

        sessionsPerDay[dateStr] = dailySessionCount;
        sessionMetrics.add(
          SessionMetric(date: date, sessionCount: dailySessionCount),
        );
        earningMetrics.add(EarningMetric(date: date, amount: dailyEarnings));
      }
